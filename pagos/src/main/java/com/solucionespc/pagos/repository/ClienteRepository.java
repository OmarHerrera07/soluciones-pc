...
public interface ClienteRepository extends JpaRepository<Cliente, Integer> {
        ...
        /**
         * Realiza una paginación de clientes con opciones de filtrado por nombre y
         * colonia.
         *
         * @param nombre    El nombre del cliente a filtrar (ignora mayúsculas y
         *                  minúsculas).
         * @param idColonia El ID de la colonia para filtrar clientes o null para
         *                  ignorar este filtro.
         ...
         */
        @Query(value = "select c.id_cliente as idCliente, c.nombre,c.telefono,p.precio as paquete,c.fecha_pago as fechaPago,c.ultimo_pago as ultimoPago, c.estado, c.abono from cliente c join paquete_internet p ON c.id_paquete = p.id_paquete WHERE LOWER(c.nombre) LIKE CONCAT('%', LOWER(?1), '%') and (c.id_colonia = ?2 or ?2 is null) order by TRIM(c.nombre)", countQuery = "select COUNT(*) from cliente c join paquete_internet p ON c.id_paquete = p.id_paquete WHERE LOWER(c.nombre) LIKE CONCAT('%', LOWER(?1), '%') and (c.id_colonia = ?2 or ?2 is null)", nativeQuery = true)
        Page<ClienteDTO> paginacionCliente(String nombre, Integer idColonia, Pageable pageable);

        /**
         * Realiza una consulta de prueba para recuperar información de clientes y
         * paquetes de Internet.
         *
         * @return Lista de objetos ClienteDTO que representan la información de
         *         clientes y paquetes.
         */
        @Query(value = "select c.nombre,c.telefono, c.fecha_pago as fechaPago,p.precio as paquete from cliente c join paquete_internet p on c.id_paquete = p.id_paquete", nativeQuery = true)
         ...
         */
        @Query(value = "SELECT c.fecha_pago FROM cliente c WHERE id_cliente = ?1", nativeQuery = true)
        ...
        /**
         * Procedimiento almacenado que inserta un mes de pago para un cliente
         * específico.
         *
        ...
        void InsertarMesPago(@Param("p_id_cliente") Integer clienteId, @Param("p_id_paquete") Integer idPaquete,
                        @Param("p_id_pago") Integer idPago, @Param("p_fecha") String fecha);

        /**
         * Obtiene un informe de clientes con pagos atrasados.
         *
         * @return Lista de objetos ReporteCliente que representan el informe de
         *         clientes.
         */
        @Query(value = "SELECT " +
                        "c.nombre,c.fecha_pago as fechaPago,c2.colonia," +
                        "DATEDIFF(NOW(), fecha_pago) AS diasAtraso, c.telefono " +
                        "FROM " +
                        "cliente c join colonia c2 on c.id_colonia = c2.id_colonia " +
                        "WHERE " +
                        "fecha_pago < CURDATE() and c.estado = 1 ORDER by c2.colonia , DATEDIFF(NOW(), fecha_pago) DESC;", nativeQuery = true)
         ...
         */
        @Query(value = "select c.abono from cliente c where c.id_cliente = ?1", nativeQuery = true)
