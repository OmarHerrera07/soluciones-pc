...
public interface PagoRepository extends JpaRepository<Pago, Integer> {
	 ...
	 */
	@Query(value = "select c.rfc,u.nombre as nombreUsuario,p.fecha,p.total,p.abono,p.tipo_ticket as tipoTicket  from pago p join cliente c on p.id_cliente = c.id_cliente join usuario u on u.id_usuario = p.id_usuario where p.id_pago =?1", nativeQuery = true)
